...
class CustomMessageTextView: UIView {
    ...
    @IBAction func onSendBtnClicked(_ sender: Any) {
        ...
        //Apply objectionable content filter
        var filteredMessage = ""
        for (index, word) in message.components(separatedBy: .whitespaces).enumerated() {
            ...
            if word.isEmpty {
                //Empty word is white space
            } else {
                if Constants.objectionableContentWordList.contains(word.lowercased()) {
                    filteredMessage += String(repeating: "*", count: word.count)
                } else {
        ...
        self.delegate?.onSendBtnClicked(message: filteredMessage)
    }
