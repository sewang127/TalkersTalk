...
class MessagingResponseAPIs: TCPConnectionServiceDelegate {
    ...
    */
    ...
    func receivedJoinChatRoomStatus(strangerId: String?, strangerName: String?, status: Constants.MessagingResponseStatus, error: Constants.MessagingJoiningChatRoomResponseError?) {
        ...
        if status == .Success {
            SocketConnectionManager.sharedInstance.strangerName = strangerName ?? "Stranger"
            
            let userName = strangerName ?? "Stranger"
            let userId = strangerId ?? ""
            
            //If Duplicate found, remove it
            for index in 0..<SocketConnectionManager.sharedInstance.strangerIdHistory.count {
                if SocketConnectionManager.sharedInstance.strangerIdHistory[index].userId == userId {
                    SocketConnectionManager.sharedInstance.strangerIdHistory.remove(at: index)
                    break
                }
            }
            
            let userTuple = (userId, userName)
            
            SocketConnectionManager.sharedInstance.strangerIdHistory.append(userTuple)
        }
    }
